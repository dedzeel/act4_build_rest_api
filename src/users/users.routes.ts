...
userRouter.post("/register", async(req : Request, res : Response) => {
    try{
        ...
        if(!username || !email || !password) {
            res.status(StatusCodes.BAD_REQUEST).json({error : `Please provide all the parameters...`})
        }
...
userRouter.post("/login", async (req : Request, res : Response) => {
    try {
        ...
        if (!user) {
             res.status(StatusCodes.NOT_FOUND).json({error : "No user exists with the emailprovided.."})
        }
